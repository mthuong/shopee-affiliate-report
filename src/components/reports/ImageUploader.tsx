...
export function ImageUploader({ onFilesSelected, pendingCount }: Props) {
  ...
  return (
    <div
      className={`border-2 border-dashed rounded-card p-8 text-center cursor-pointer transition-colors ${dragging ? 'border-accent bg-accent/5' : 'border-line-strong hover:border-muted'}`}
    ...
    >
      ...
      <p className="text-muted text-xs mt-1">Supports JPG, PNG</p>
    </div>
  )
}
